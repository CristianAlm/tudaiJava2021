...
public class ejer14 {
	...
	private static void recorrerArray(char[] arr, char[] patron, int inicio, int fin, int inicioPatron, int finPatron) {
		...
		while (posIni<COLUMNAS) {
			...
			if (posIni!=-1) {
				...
				if (posFin!=-1) {
					if(esValido(arr,posIni, posFin, patron,inicioPatron,finPatron)){
						
						//System.out.println("misma longitud");
						if(comparar(arr,patron, posIni,posFin, inicioPatron,finPatron)){
	...
	private static boolean comparar(char[] arr, char[] patron, int posIni,
			int posFin, int inicioPatron, int finPatron) {
		// TODO Auto-generated method stu
		
		int max = 0;
		for(int i =0; i<(finPatron-inicioPatron+1);i++){
			if(arr[posIni+i] == patron[inicioPatron+i]){
				max++;
			}else{
				return false;
			}
		}
		if(max==(finPatron-inicioPatron+1)){
			return true;
		}else{
			return false;
		}
	...
	private static boolean esValido(char[] arr, int posIni, int posFin,char[] patron, int inicioPatron, int finPatron) {
		// TODO Auto-generated method stub
		if((posFin-posIni+1)== (finPatron-inicioPatron+1)){
			return true;		
		}
		return false;
	}
